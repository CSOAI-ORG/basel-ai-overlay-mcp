...
mcp = FastMCP(
    "Basel III + AI Model Risk Management",
    instructions=(
        "By MEOK AI Labs — Basel III + SR 11-7 + ECB TRIM AI/ML model risk management for banks deploying AI in credit, capital, treasury, AML/CFT. "
...
_UPSELL = (
    ...
    "   ⭐ Free & open-source → https://github.com/CSOAI-ORG/basel-ai-overlay-mcp"
)
...
@mcp.tool()
def model_risk_tier(query: str = "", api_key: str = "") -> str:
    """Tier AI/ML models per SR 11-7 materiality framework
    ...
    return json.dumps({
        "tool": "model_risk_tier",
        "query": query,
        "status": "stub",
        "tool_description": "Tier AI/ML models per SR 11-7 materiality framework",
...
@mcp.tool()
def validation_requirements(query: str = "", api_key: str = "") -> str:
    """Independent model validation requirements + frequency
    ...
    return json.dumps({
        ...
        "query": query,
        "status": "stub",
        "tool_description": "Independent model validation requirements + frequency",
        "note": "Initial scaffold v1.0.0 — extended logic ships in v1.1 with real regulation data ingestion.",
        "regulation_refs": [],
        "next_step": "POST result to https://meok-attestation-api.vercel.app/sign for HMAC-signed compliance cert",
        "tier": tier,
...
@mcp.tool()
def ecb_trim_check(query: str = "", api_key: str = "") -> str:
    """ECB TRIM internal model investigation areas
    ...
    return json.dumps({
        ...
        "query": query,
        "status": "stub",
        "tool_description": "ECB TRIM internal model investigation areas",
        "note": "Initial scaffold v1.0.0 — extended logic ships in v1.1 with real regulation data ingestion.",
        "regulation_refs": [],
        "next_step": "POST result to https://meok-attestation-api.vercel.app/sign for HMAC-signed compliance cert",
        "tier": tier,
...
@mcp.tool()
def ifrs9_ecl_audit(query: str = "", api_key: str = "") -> str:
    """IFRS 9 Expected Credit Loss AI model governance
    ...
    return json.dumps({
        "tool": "ifrs9_ecl_audit",
        "query": query,
        "status": "stub",
        "tool_description": "IFRS 9 Expected Credit Loss AI model governance",
        "note": "Initial scaffold v1.0.0 — extended logic ships in v1.1 with real regulation data ingestion.",
        "regulation_refs": [],
        "next_step": "POST result to https://meok-attestation-api.vercel.app/sign for HMAC-signed compliance cert",
        "tier": tier,
...
@mcp.tool()
def frtb_market_risk(query: str = "", api_key: str = "") -> str:
    """FRTB internal model approach AI/ML usage check
    ...
    return json.dumps({
        "tool": "frtb_market_risk",
        "query": query,
        "status": "stub",
        "tool_description": "FRTB internal model approach AI/ML usage check",
        "note": "Initial scaffold v1.0.0 — extended logic ships in v1.1 with real regulation data ingestion.",
        "regulation_refs": [],
        "next_step": "POST result to https://meok-attestation-api.vercel.app/sign for HMAC-signed compliance cert",
        "tier": tier,
